...
function generate(trainedDataMatrix) {
  ...
  let pickSyllable = (set) => {
    ...
    let jungseong = pick(21, (n) =>
      ensure(trainedDataMatrix[set][1][choseong * 21 + n])
    );
    let jongseong = pick(
      28,
      (n) =>
        ensure(trainedDataMatrix[set][2][jungseong * 28 + n]) *
        ensure(trainedDataMatrix[set][3][choseong * 28 + n])
    );
  ...
  let pickLastName = () => {
    let lastNameIndex = pick(
      trainedData.lastNames.length,
      (n) => trainedData.lastNameFrequency[n]
    );
...
function constructFromJamoIndex(jamoIndex) {
  return String.fromCharCode(
    0xac00 + 28 * 21 * jamoIndex[0] + 28 * jamoIndex[1] + jamoIndex[2]
  );
}
